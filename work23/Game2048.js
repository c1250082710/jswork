(function (window, document, $) {
    ...
    Board.prototype = {
      ...
      generate: function () {
        ...
        var pose = '_'+this.score+'_.' +  'this.len  .'
        ...
        for (var x = 0, len = this.arr.length; x < len; ++x) {
          for (var y = len - 1, arr = this.arr[x]; y >= 0; --y) {
            for (var prev = y - 1; prev >= 0; --prev) {
              if (arr[prev] === 0) continue;
              if (arr[y] === 0) {
                arr[y] = arr[prev];
                this.onMove({from: {x: x, y: prev, num: arr[prev]},
      ...
      ranklist: function (url, user, score, view) {
        $.post(url, {name: user, score: score}, function (data) {
          let str = score + '<br>'
          str += data.map((val, index) => {
            return index + 1 + " " + val.name + "-" + val.score
          }).slice(0, 3).join('<br>')
          setTimeout(function () {
            view.over(str)
          }, 1000)
    ...
    View.prototype = {
      ...
      init: function () {
        for (var x = 0, len = this.len; x < len ;++x) {
          for (var y = 0 ;y < len; ++y) {
            ...
            $cell.css({width: this.size + 'px', height: this.size + 'px', top: this.getPos(x), left: this.getPos(y)
      ...
      addNum: function (x, y, num) {
        ...
        $num.text(num).css({top: this.getPos(x) + parseInt(this.size / 2), left: this.getPos(y) + parseInt(this.size / 2)}).appendTo(this.container).animate({width: this.size + 'px', height: this.size + 'px', lineHeight: this.size + 'px', top: this.getPos(x), left: this.getPos(y)
        }, 100)
        this.nums[x + '-' + y] = $num
      },
